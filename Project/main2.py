...
from NBClassifier import normalize_features, train_and_evaluate_classifier
import numpy as np
...
def main():
    window_sizes = list(range(1000, 12000 + 1, 1000))
    overlap_values = [0.25, 0.5, 0.75] # Modify as you wish
    feature_sets = ["time", "freq"] # Feature sets to test
    ...
    # Plot 9/12 graphs: for each feature set and each overlap
    for fs in feature_sets:
        for ov in overlap_values:
            ...
            plt.tight_layout()
            save_path = Path.home() / "Downloads" / f"accuracy_{fs}_{ov}.png"
